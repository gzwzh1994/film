...
public class MoiveAction extends ActionSupport implements ModelDriven<Moive> {
	...
	public String add() throws Exception {
		HttpServletRequest request = ServletActionContext.getRequest();
		if (request.getParameter("moivename") == null
				|| request.getParameter("moivename") == "") {

		} else {
			moive.setMoivename(request.getParameter("moivename"));
			moive.setReleasetime(Date.valueOf(request
			...
			Object[] param = { moive.getMoivename(), moive.getReleasetime(),
					moive.getDirector() };
			Moive m = null;
			m = moiveManager
					.findMoive(
							"from Moive m where m.moivename=? and m.releasetime=? and m.director=?",
							param);
			if (null == m) {
				String realpath = ServletActionContext.getServletContext()
						.getRealPath("/Upload/FilmPic/Temp");
