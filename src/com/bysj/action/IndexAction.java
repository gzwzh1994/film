...
public class IndexAction extends ActionSupport implements ModelDriven<Moive> {
	...
	public String I() {
		...
		List<MoiveIndex> l1 = moiveManager
				.findListByDate(
						"select new com.bysj.vo.MoiveIndex(m.moiveid,m.moivename,m.releasetime,m.score,i.imageadr) from Moive m,Image i where m.releasetime<? and i.imagename='FilmPic' and m.moiveid=i.moiveid",
						param);
		List<MoiveIndex> l2 = moiveManager
				.findListByDate(
						"select new com.bysj.vo.MoiveIndex(m.moiveid,m.moivename,m.releasetime,m.score,i.imageadr) from Moive m,Image i where m.releasetime>? and i.imagename='FilmPic' and m.moiveid=i.moiveid",
						param);
