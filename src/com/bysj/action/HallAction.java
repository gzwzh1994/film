...
public class HallAction extends ActionSupport{
	...
	public String add() throws Exception{
		...
		if(hallname==null || hallname==""){
			...
			h=hallManager.findHall("from Hall h where h.hallname=? and h.cinemaid=?", param);
			if(null==h){
	...
	public String update() throws Exception{
		...
		if(hallname==null || hallname==""){
			...
			if(null==h){
				...
				hall=hallManager.findHall("from Hall h where h.hallid=?", param1);
				hall.setHallname(hallname);
				hall.setCinemaid(Integer.parseInt(request.getParameter("cinemaid")));
