...
public class UserAction extends ActionSupport implements ModelDriven<UserInfo>{
    ...
    public String login() throws Exception{
    	...
    	if(!yzm.equals(authCode)){
    		...
    		User u=null;
    	    u=userManage.findUserByPhone(Long.parseLong(userinfo.getTxtMobile()));
    	    if(null==u){
    	    	request.setAttribute("errorTip","登录失败：用户不存在");
    	    	return ERROR;
    	    }else{
    	    	u=userManage.findUserByPhoneAndPassword(Long.parseLong(userinfo.getTxtMobile()), userinfo.getTxtPwd());
    	    	if(null==u){
    ...
    public String Update() throws Exception{
    	...
    	if(oldpwd.equals("")){
        	...
        	userManage.updateUserhql("update User u set u.username=? where u.phone=?", param);
    		...
    		if(user.getUserpwd().equals(oldpwd)){
    			...
    			userManage.updateUserhql("update User u set u.userpwd=? where u.phone=?", param);
    ...
    public String manage(){
		HttpServletRequest request = ServletActionContext.getRequest();
		List<User> userlist=userManage.findAll("select new User(u.userid,u.phone,u.username) from User u");
