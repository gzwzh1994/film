...
public class CinemaAction extends ActionSupport{
	...
	public String add() throws Exception{
		...
		if(cinemaname==null || cinemaname==""){
			...
			c=cinemaManager.findCinema("from Cinema c where c.cinemaname=?", param);
			if(null==c){
				cinema.setCinemaname(cinemaname);
				cinema.setCityid(Integer.parseInt(request.getParameter("cityid")));
				...
				cinema.setPreferentialinfo(request.getParameter("preferentialinfo"));
	...
	public String update() throws Exception{
		...
		if(cinemaname==null || cinemaname==""){
			...
			cinema=cinemaManager.findCinema("from Cinema c where c.cinemaid=?", param);
			List<City> citylist=citymanager.findAllList("from City");
	    	request.setAttribute("citylist", citylist);
			...
			c=cinemaManager.findCinema("from Cinema c where c.cinemaname=? and c.cityid=?", param);
			if(null==c){
				...
				cinema=cinemaManager.findCinema("from Cinema c where c.cinemaid=?", param1);
				List<City> citylist=citymanager.findAllList("from City");
		    	request.setAttribute("citylist", citylist);
				request.setAttribute("cinema", cinema);
