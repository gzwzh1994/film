...
public class OrderAction extends ActionSupport {
	...
	public void CreateSeatTicket() throws Exception{
		...
		String authCode=session.getAttribute("authCode").toString().toLowerCase();
		if(!code.equals(authCode)){
			...
			Integer scheduleid=Integer.parseInt(request.getParameter("seqNo"));
			...
			List<MoiveInfo2> l=scheduleManager.findScheduleBy("select new com.bysj.vo.MoiveInfo2(m.moiveid,m.moivename,i.imageadr,m.totaltime,s.showdate,s.showtime,s.versionid,m.language,c.cinemaid,c.cinemaname,h.hallname,s.price) from Schedule s,Moive m,Hall h,Cinema c,Image i where s.scheduleid=? and s.moiveid=m.moiveid and m.moiveid=i.moiveid and s.hallid=h.hallid and h.cinemaid=c.cinemaid and i.imagename='FilmPic' ", param);
			String leix = "";
			...
			String details = "影片：" + l.get(0).getCFilmName() + "<br/>影院：" + l.get(0).getCinemaName() + "<br/>影厅：" 			    
			                   + l.get(0).getHallName() + "<br/>类型："+ leix + "<br/>场次："+l.get(0).getShowDate()+" "+l.get(0).getShowTime()+"<br/>座位：";
			...
			String jsonArray="{\"errMsg\":\""+order.getOrderid()+"&seatName="+seatName+"\",\"errorCode\":\"0\"}";
			response.setCharacterEncoding("utf-8");
			PrintWriter out = response.getWriter();
			out.print(jsonArray);
			out.flush();
	...
	public String manage(){
		...
		List<User> userlist=userManage.findAll("select new User(u.userid,u.phone) from User u");
